...
def models(paramGamma, paramMax_episodes, paramHidden_dims, paramLr):
    class FCQ(nn.Module):
        '''완전연결 신경망 클래스'''
    ...
    environment_settings = {                # 환경 설정 매개변수
        ...
        "gamma" : paramGamma,               # 할인율, float 형으로 작성
        "max_episodes" : paramMax_episodes  # 최대 에피소드 수, int 형으로 작성
    }

    # 신경망 모델 구성, dtype: lambda, (30, ), 은닉계층 차원 설정
    value_model_fn = lambda nS, nA: FCQ(nS, nA, hidden_dims=(paramHidden_dims,))
    # 신경망 최적화 함수, dtype: lambda, 파라미터 설정 및 학습률 지정
    value_optmizer_fn = lambda net, lr: optim.RMSprop(net.parameters(), lr=lr)
    value_optmizer_lr = paramLr   # 학습률 지정
    ...
    print("")
    print("보상 최대값: ", max(reward_track))
    print("보상 최소값: ", min(reward_track))
